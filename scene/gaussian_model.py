...
# import pytorch3d.transforms as pt3d_T
import os
...
# from scene.non_rigid_deformer import NonRigidDeformer
# from scene.fourdgs_deform import deform_network
...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, smpl_type : str, motion_offset_flag : bool, actor_gender: str):
        ...
        self.lweight_offset_decoder = None
    ...
    def training_setup(self, training_args):
        ...
        if self.motion_offset_flag:
            l.append({'params': self.pose_decoder.parameters(), 'lr': training_args.pose_refine_lr, "name": "pose_decoder"})
            l.append({'params': self.lweight_offset_decoder.parameters(), 'lr': training_args.lbs_offset_lr, "name": "lweight_offset_decoder"})
        ...
        self.deformation_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init * self.spatial_lr_scale,
                                                    lr_final=training_args.position_lr_final * self.spatial_lr_scale,
                                                    lr_delay_mult=training_args.position_lr_delay_mult,
                                                    max_steps=training_args.position_lr_max_steps)
    ...
    def update_learning_rate_stage2(self, iteration):
        ''' Learning rate scheduling per step '''
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                lr = self.xyz_scheduler_args(iteration)
    ...
    def densify_and_prune_3dgs(self, max_grad, min_opacity, extent, max_screen_size):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0
    ...
    def add_densification_stats_3dgs(self, viewspace_point_tensor, update_filter):
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter,:2], dim=-1, keepdim=True)
        self.denom[update_filter] += 1


    def coarse_deform_c2source_origin(self, query_pts, params, t_params, t_vertices, lbs_weights=None, correct_Rs=None, return_transl=False):
    ...
    def coarse_deform_c2source(self, query_pts, smpl_shape, smpl_pose, smpl_R, smpl_Rh, smpl_Th, t_params, t_vertices, lbs_weights=None, correct_Rs=None, return_transl=False, view_camera=None):
        ...
        segs = torch.argmax(bweights, dim=-1, keepdim=True)
        segs = segs.detach()  # [blur_num, point_num, 1]

        # assert False

        ### From Big To T Pose
        big_pose_params = t_params
        A, R, Th, T_joints = get_transform_params_torch(self.SMPL_NEUTRAL, big_pose_params)
        A = torch.matmul(bweights, A.reshape(1, joints_num, -1))
        
        A = torch.reshape(A, (bs, -1, 4, 4))  # [5, 6890, 4, 4]
        ...
        if self.mean_shape:
            ...
            posedirs = self.SMPL_NEUTRAL['posedirs'].cuda().float()
            pose_ = big_pose_params['poses'].repeat(bs, 1)
            ident = torch.eye(3).cuda().float()
            batch_size = pose_.shape[0]
            rot_mats = batch_rodrigues(pose_.view(-1, 3)).view([batch_size, -1, 3, 3])
            pose_feature = (rot_mats[:, 1:, :, :] - ident).view([batch_size, -1])#.cuda()
            ...
            # From mean shape to normal shape
            shapedirs = self.SMPL_NEUTRAL['shapedirs'][..., :smpl_shape.shape[-1]]#.cuda()
            shape_offset = torch.matmul(shapedirs.unsqueeze(0), torch.reshape(smpl_shape.cuda(), (batch_size, 1, -1, 1))).squeeze(-1)
            shape_offset = torch.gather(shape_offset, 1, vert_ids.expand(-1, -1, 3)) # [bs, N_rays*N_samples, 3]
            query_pts = query_pts + shape_offset
        ...
        # get tar_pts, smpl space source pose
        A, R, Th, joints = get_transform_params_torch_modify(
            self.SMPL_NEUTRAL, smpl_shape, smpl_pose, smpl_R, smpl_Th, rot_mats=rot_mats)
        ...
        world_src_pts = torch.matmul(smpl_src_pts, R_inv) + Th[:, None]

        if R.dim() == 3:
            transforms = torch.matmul(R[:, None], transforms)
        else:
            transforms = torch.matmul(R, transforms)
        ...
        return smpl_src_pts, world_src_pts, bweights, transforms, translation, T_joints
...
def get_rigid_transformation_torch(rot_mats, joints, parents):
    ...
    transforms = torch.cat([transforms[..., :3], (transforms[..., 3] - rel_joints).unsqueeze(-1)], dim=-1)

    return transforms



def get_transform_params_torch_modify(smpl, smpl_shape, smpl_pose, smpl_R, smpl_Th, rot_mats=None, correct_Rs=None):
    ...
    # print(v_template[None].repeat(smpl_shape.shape[0], 1, 1).shape)
    # print(shapedirs[None].repeat(smpl_shape.shape[0], 1, 1, 1)[...,:betas.shape[-1]].shape)
    # print(betas[:, None, None].shape)

    v_shaped = v_template[None].repeat(smpl_shape.shape[0], 1, 1) + torch.sum(shapedirs[None].repeat(smpl_shape.shape[0], 1, 1, 1)[...,:betas.shape[-1]] * betas[:, None, None], axis=-1).float()

    if rot_mats is None:
        # add pose blend shapes
        poses = smpl_pose.reshape(-1, 3)
        # bs x 24 x 3 x 3
        rot_mats = batch_rodrigues_torch(poses).view(smpl_pose.shape[0], -1, 3, 3)
...
def get_covariance(pc: GaussianModel, scaling_modifier, transform, nonrigid_scaling, nonrigid_rotation):
    return build_covariance_from_scaling_rotation(pc.scaling_activation(nonrigid_scaling), scaling_modifier, nonrigid_rotation, transform)
